...
> Status: Living Document | Last updated: 2026-02-10
...
| **termLower** | ⚙️ *Portal convention*. A lowercased copy of the `term` field stored in the search index at build time. Used for case-insensitive exact matching because Orama's `exact: true` is case-sensitive against stored values. The original `term` field preserves display casing (e.g., "Wallet Unit Attestation WUA"); `termLower` stores the lowercase equivalent for matching queries like "wua". Generated in `build-search-index.js`. |
| **Exact Match Boost** | The search strategy that runs a secondary `exact: true` search on `termLower` to find abbreviation matches. Results from this search are placed BEFORE fuzzy results, ensuring terminology definitions appear at the top when users search for abbreviations. |
...
| **Intended Use** | A formally registered declaration at the Registrar specifying *why* an RP requests data and *which* data elements it needs. Each intended use has a unique identifier (AS-MS-27-039) and a user-friendly description shown to wallet users for informed consent (EW-DM-44-018 field (c)). Intended uses are bound to registration certificates — one certificate per intended use — and the wallet may verify that requested attributes match the registered scope (EW-DM-44-021). An RP may register multiple intended uses. No single regulation defines the term explicitly; it is assembled from IR 2025/848 Art 5, ARF Topic 27 (AS-MS-27-038, AS-MS-27-039), and Topic 33 (EW-DM-44-018, EW-DM-44-021). Source: synthesized. |
...
| **Access Certificate Authority (ACA)** | Entity authorised by a Member State (IR 2025/848 Article 7) to issue wallet-relying party access certificates. ACA certificate policy must comply with ETSI TS 119 411-8 and at least the NCP requirements of ETSI EN 319 411-1. Each Member State must authorise at least one ACA. |
| **STS Repo** | `eu-digital-identity-wallet/eudi-doc-standards-and-technical-specifications` — the canonical GitHub repository containing the full text of all 14 Technical Specifications (TS01–TS14) referenced by the ARF. All TS deep links should point here, not the ARF repo. |
| **ARF Repo** | `eu-digital-identity-wallet/eudi-doc-architecture-and-reference-framework` — the architecture framework repository. Its `docs/technical-specifications/` directory contains only stub redirect files pointing to the STS repo, NOT actual spec content. The HLR CSV and main framework text live here. |
...
| **RP Intermediary** | ⚠️ **Updated per DEC-254**: A unified intermediary type that acts on behalf of Relying Parties to interact with EUDI Wallets. Per ARF Topic 52 (AS-RP-51-001-10), RP Intermediaries perform BOTH forwarding AND verification functions as a single role. The previous PIF/VIF distinction has been retired — it was not official terminology. Requirements prefix: `VEND-INT-*`. Legal basis: Article 5b(10). |
...
| **HLR** | High-Level Requirement. The technical requirements defined in the ARF (Architecture Reference Framework) that specify how EUDIW components should behave. Each HLR has a unique ID (e.g., `AS-RP-51-008`, `EW-PIO-01-020`) and belongs to a numbered Topic (1-55). VCQ requirements reference HLRs via the `arfReference` field. The portal imports 143 HLRs from 6 relevant topics. |
| **TS7** | Technical Specification 7 — the ARF specification defining the common interface for data deletion requests. Implements GDPR Article 17 (right to erasure) for the EUDI Wallet ecosystem. RPs and intermediaries must support TS7 to process user-initiated data deletion requests. VCQ requirements `VEND-CORE-021` and `VEND-INT-030` mandate TS7 compliance. See ARF Topic 48. |
| **ARF Topic 48** | The ARF topic covering data deletion requirements (`DATA_DLT_*` HLRs). Key HLRs: `AS-RP-48-005` (user authentication before deletion), `AS-WP-48-004` (TS7 compliance). Most Topic 48 HLRs apply to Wallet Providers (excluded from VCQ); only `AS-RP-48-005` and `AS-WP-48-004` apply to RP/Connector vendors. |
...
| **Disposition (Audit)** | A classification assigned to each RCA requirement during cross-tool audits. Values: `V` (Vendor-Applicable — must be in VCQ), `O` (Organization-Only — intentionally excluded), `X` (Already in VCQ — map to existing), `P` (Partial — split required). Used in RCA↔VCQ gap analysis. |
| **Vendor-Applicable Requirement** | An RCA requirement that can be implemented or verified in a vendor product. Contrasted with Organization-Only requirements which require organizational action (registration with authorities, HR, financial). VCQ covers only vendor-applicable requirements. |
| **Organization-Only Requirement** | An RCA requirement that requires organizational action with external authorities or internal governance — cannot be delivered by a vendor product. Examples: registration with supervisory bodies, maintaining personnel expertise, financial resources. These are intentionally excluded from VCQ and documented in the exclusion registry. |
| **Public-Sector-Only Requirement** | An RCA requirement that applies only to public sector bodies (e.g., government agencies). VCQ targets private sector commercial products, so these are excluded from VCQ scope. Examples: mandatory EUDI wallet acceptance for public online services, cross-border e-ID recognition. |
| **RCA↔VCQ Scope Boundary** | The formal definition distinguishing which requirements belong in RCA (organizational compliance) vs VCQ (vendor product evaluation). Documented in KB artifact `rca_vcq_scope_boundary.md`. Key principle: if an obligation requires action with authorities or internal governance, it stays in RCA only. |
| **OIDF Conformance Suite** | The OpenID Foundation's free, open-source conformance testing infrastructure for validating protocol-level correctness of OID4VP (verifier) and OID4VCI (issuer) implementations. Available at `demo.certification.openid.net` and as a self-hosted Docker deployment from [GitLab](https://gitlab.com/openid/conformance-suite/). Tests cover SD-JWT VC and ISO mDoc credential formats. Not to be confused with EU-level wallet certification (ENISA-managed), which targets Member State wallets, not vendor products. VCQ requirements: VEND-CORE-043 (OID4VP), VEND-CORE-044 (OID4VCI). |
| **TS5** | Technical Specification 5 — "Common Formats and API for RP Registration Information" (EC TS05 V1.0, June 2025). Defines the standardized REST API (OpenAPI v3) and JSON data formats for exchanging registration information between Relying Parties and Member State registries. Key features: JWS-signed responses (IETF RFC 7515), public read access, query parameters for registration number/company name/entitlements/intermediary flag. Legal basis: Implementing Regulation 2025/848. VCQ requirement: VEND-CORE-045. |
| **Exemplar Standard (VCQ)** | The target quality level for VCQ requirement explanations and clarification questions, established during the 147-requirement audit (2026-02-10). Requirements at exemplar standard have: multi-paragraph explanations with bold section headers, specific article+paragraph references, legalText field populated with direct quotes, structural fixes (no empty `paragraph: ""` fields), cross-references to related requirements, and exactly 9 clarification questions per requirement covering diverse assessment dimensions. Templates: `.agent/snippets/vcq-requirement-template.md` and `.agent/snippets/vcq-question-template.md`. |
| **Dimension (VCQ Clarification)** | The assessment category assigned to each clarification question in the VCQ questionnaire. Ensures questions probe diverse aspects of vendor compliance beyond surface-level answers. 17 standard dimensions: `compliance_completeness`, `compliance_monitoring`, `compliance_evidence`, `operational`, `security`, `sla`, `documentation`, `interoperability`, `incident_response`, `commercial`, `flexibility`, `lifecycle`, `supply_chain`, `governance`, `regulatory_access`, `experience`, `roadmap`. Each requirement's 9 questions should cover at least 7 distinct dimensions. |
| **Dual Regime** | A situation where two regulatory frameworks apply simultaneously to the same vendor/service. In the VCQ context, this occurs when: (1) eIDAS and DORA apply to ICT services for financial entities (VEND-ICT track), (2) eIDAS and GDPR apply to data processing obligations, or (3) DORA and GDPR create parallel incident notification requirements (Article 30(2)(f) + GDPR Article 33(2)). Contracts must satisfy both sets of requirements. |
| **CTPP** | Critical ICT Third-Party Provider. An ICT service provider designated by the ESAs (EBA, ESMA, EIOPA) under DORA Article 31 as systemically important to the financial sector. Designation criteria: systemic importance, degree of substitutability, number of dependent financial entities, cross-border dependencies. Designated CTPPs are subject to enhanced oversight including direct supervision by a Lead Overseer. See VEND-ICT-012. |
| **TLPT** | Threat-Led Penetration Testing. A structured, intelligence-driven security testing methodology required by DORA Article 26(4) for identified financial entities. Built on the TIBER-EU framework. ICT TPPs must cooperate with TLPT when their services are in scope. Key features: threat intelligence-based scenario design, red team testing by accredited providers, results shareable with competent authorities, 3-year testing cycle, pooled testing option. See VEND-ICT-008. |
| **Sole Control (Trust Services)** | The eIDAS requirement that electronic signature creation can only be triggered by the signatory — even the QTSP itself cannot initiate signature creation without explicit signatory authorisation. Implementation requires multi-factor authentication (MFA), Signature Activation Data (SAD), explicit consent protocols, and anti-replay mechanisms. See VEND-TSP-009. |
| **WYSIWYS** | What You See Is What You Sign. The eIDAS principle requiring that the document presented to the signatory for review is identical to the document that will be signed. Implementation must address: document rendering fidelity, prevention of dynamic content (JavaScript, macros), format stability, accessibility compliance, and mobile rendering consistency. Critical for remote signing via QTSP. See VEND-TSP-010. |
| **Dual Legal Basis (VCQ)** | ⚙️ *Portal convention*. A VCQ requirement whose `legalBasis` field contains two entries: one from the **mandating** regulation (eIDAS/implementing act) that creates the obligation, and one from the **operative** regulation (GDPR) specifying the substantive requirement to comply with. Example: VEND-CORE-002 has eIDAS Art 5a(17) (mandate) + GDPR Art 28(3) (operative). Not all GDPR requirements have dual bases — some are standalone GDPR obligations applicable to vendors in their own right. |
| **Art 5a(17) Cross-Reference** | The provision in Regulation (EU) 2014/910 (as amended) that mandates GDPR compliance for personal data processing in the EUDI Wallet ecosystem: *"Any processing of personal data [...] shall be carried out in accordance with appropriate and effective data protection measures. Compliance of such processing with Regulation (EU) 2016/679 shall be demonstrated."* Used as the eIDAS legal basis for GDPR processor obligations (Art 28), security measures (Art 32), and breach notification (Art 33) when those obligations arise specifically from wallet intermediary data processing. |
| **Deployment Architecture (VCQ)** | ⚙️ *Portal convention (DEC-289)*. How a vendor's connector product integrates with the customer's RP infrastructure. Three deployment models: `intermediary` (vendor acts as RP on behalf of customer), `direct_saas` (vendor provides hosted connector, customer is the RP), `direct_onprem` (vendor provides software, customer deploys on own infrastructure). Stored in `deploymentArchitectures` array field on VCQ requirements. Requirements without this field are **agnostic** — they apply to all architectures. |
| **Intermediary Architecture** | A deployment model where the vendor acts as a third-party RP on behalf of the integrating organisation. The vendor holds its own Access Certificate and interacts with wallets directly. Triggers requirements around Article 5b(10) no-storage mandate, dual registration, and intermediary-specific governance. Badge: `INT` (blue). |
| **Direct SaaS Architecture** | A deployment model where the organisation is the RP and the vendor provides a hosted cloud connector service. The vendor manages infrastructure but the organisation maintains its own RP registration and Access Certificates. Triggers requirements around data residency, processor agreements, and multi-tenancy isolation. Badge: `SaaS` (purple). |
| **Direct Self-Hosted Architecture** | A deployment model where the organisation is the RP and deploys the vendor's software on its own infrastructure (on-premises or private cloud). The vendor delivers binaries/containers; the organisation manages all operations. Triggers requirements around update/patching SLAs, deployment documentation, and air-gapped installation support. Badge: `OnPrem` (emerald). |
| **Agnostic Requirement (VCQ)** | A VCQ requirement that applies regardless of deployment architecture. These have an empty or absent `deploymentArchitectures` array and are always shown when the architecture filter is active. Examples: cryptographic algorithm support, EUDI Wallet protocol compliance. Contrast with architecture-specific requirements that have `deploymentArchitectures: [intermediary]` etc. |
| **Architecture Badge** | A compact, color-coded label displayed next to a VCQ requirement ID indicating which deployment architecture(s) the requirement targets. Three badges: **INT** (blue, intermediary), **SaaS** (purple, direct SaaS), **OnPrem** (emerald, self-hosted). Agnostic requirements show no badge. Badges appear in the requirements table and in Markdown/Excel exports. Added in DEC-289. |
...
| **ARF Index (Old Identifier)** | The legacy requirement identifier used before ARF v2.7.0 (e.g., `AS-WP-09-026`, `AS-AP-10-048`, `AS-MS-27-018`). Still present as the `Index` column in the HLR CSV. Most cross-references in our VCQ/RCA use this format. During ARF upgrades, content may move to a different Harmonized ID while keeping the same Index, or the Index itself may change. |
...
| **Scope Rate-Limited Pseudonym** | A privacy-preserving pseudonym mechanism introduced in ARF v2.8.0 (AS-WP-11-024–AS-WP-11-032, Topic 11, from Discussion Paper E). Allows a User to generate pseudonyms that are: (a) rate-limited per scope/RP to prevent Sybil attacks, (b) unlinkable across different RPs, (c) unlinkable within the same scope when rate > 1, and (d) persistent across Wallet Unit changes. Crypto material stored in WSCA/WSCD or keystore. |
| **Old ID** | ⚙️ *Portal convention*. Informal name for the `Index` column value (e.g., `AS-WP-09-026`, `AS-AP-10-048`) used in VCQ/RCA references *before* migration to Harmonized IDs. Old IDs are non-unique across ARF versions — the same Old ID can map to different Harmonized IDs depending on the version. After the Harmonized ID migration, Old IDs remain in `arf-hlr-data.json` under `byHlrId` for backward compatibility and search display. |
| **ID Migration (ARF)** | ⚙️ *Portal convention*. The process of updating VCQ `arfReference.hlr` fields from Old IDs (e.g., `AS-AP-10-044`) to ARF Harmonized IDs (e.g., `AS-AP-10-029`). Required because Old IDs are non-unique and unstable across ARF versions. After migration, validation scripts (`validate-vcq.js`, `validate-vcq-arf.js`) check against `byHarmonizedId` instead of `byHlrId`. See Phase 2 of the ARF v2.8.0 upgrade plan. |
| **Empty Tombstone** | ⚙️ *Portal convention*. A CSV row where `Requirement_specification = "Empty"` that shares a Harmonized ID with a content-bearing row. Indicates a retired requirement that has been merged into the other row with the same Harmonized ID. In ARF v2.8.0, 5 tombstones exist in Topic 38 (e.g., `EW-DM-38-007` is a tombstone for `EW-DM-38-007`, both mapping to `EW-DM-38-007`). Tombstones come AFTER content rows in CSV order, so naïve last-write-wins loses the content — must use a content-wins guard when building `byHarmonizedId`. |
| **Content-Wins Strategy** | ⚙️ *Portal convention*. A guard in `import-arf.js` that prevents Empty tombstones from overwriting content-bearing entries in the `byHarmonizedId` index. Required because the EC CSV places tombstone rows after content rows for the same Harmonized ID. Implementation: only write to `byHarmonizedId[id]` if the slot is empty or the new row has content. See AGENTS.md § ARF Data Indices for code pattern. |
| **Semantic Topic Label** | ⚙️ *Portal convention*. The `arfReference.topic` field in VCQ YAML requirements, which indicates the ARF topic *area* the requirement relates to, rather than strictly matching the HLR's actual CSV topic number. Deep links use the HLR's CSV topic (correct destination), while the badge/label uses the VCQ topic (may differ). 19 mismatches exist across 12 VCQ requirements — all pre-existing and mostly cosmetic. Only the Topic 53↔52 mismatch has functional impact (wrong `relevantTopics` lookup). |
