...
| **Intended Use** | A formally registered declaration at the Registrar specifying *why* an RP requests data and *which* data elements it needs. Each intended use has a unique identifier (Reg_32) and a user-friendly description shown to wallet users for informed consent (RPRC_19a field (c)). Intended uses are bound to registration certificates — one certificate per intended use — and the wallet may verify that requested attributes match the registered scope (RPRC_21). An RP may register multiple intended uses. No single regulation defines the term explicitly; it is assembled from IR 2025/848 Art 5, ARF Topic 27 (Reg_31, Reg_32), and Topic 33 (RPRC_19a, RPRC_21). Source: synthesized. |
...
| **ARF Topic 48** | The ARF topic covering data deletion requirements (`DATA_DLT_*` HLRs). Key HLRs: `DATA_DLT_07` (user authentication before deletion), `DATA_DLT_08` (TS7 compliance). Most Topic 48 HLRs apply to Wallet Providers (excluded from VCQ); only `DATA_DLT_07` and `DATA_DLT_08` apply to RP/Connector vendors. |
...
| **Old ID** | ⚙️ *Portal convention*. Informal name for the `Index` column value (e.g., `WUA_20a`, `ISSU_32`) used in VCQ/RCA references *before* migration to Harmonized IDs. Old IDs are non-unique across ARF versions — the same Old ID can map to different Harmonized IDs depending on the version. After the Harmonized ID migration, Old IDs remain in `arf-hlr-data.json` under `byHlrId` for backward compatibility and search display. |
| **ID Migration (ARF)** | ⚙️ *Portal convention*. The process of updating VCQ `arfReference.hlr` fields from Old IDs (e.g., `ISSU_29`) to ARF Harmonized IDs (e.g., `AS-AP-10-029`). Required because Old IDs are non-unique and unstable across ARF versions. After migration, validation scripts (`validate-vcq.js`, `validate-vcq-arf.js`) check against `byHarmonizedId` instead of `byHlrId`. See Phase 2 of the ARF v2.8.0 upgrade plan. |
| **Empty Tombstone** | ⚙️ *Portal convention*. A CSV row where `Requirement_specification = "Empty"` that shares a Harmonized ID with a content-bearing row. Indicates a retired requirement that has been merged into the other row with the same Harmonized ID. In ARF v2.8.0, 5 tombstones exist in Topic 38 (e.g., `WURevocation_17` is a tombstone for `WURevocation_09`, both mapping to `EW-DM-38-007`). Tombstones come AFTER content rows in CSV order, so naïve last-write-wins loses the content — must use a content-wins guard when building `byHarmonizedId`. |
