...
#
# The script:
# 1. Lists all open pages via CDP
# 2. Creates a fresh about:blank tab
# 3. Closes all other tabs
#
# Usage: ./scripts/cleanup-chrome-tabs.sh
#
# Returns exit code 0 on success, 1 if Chrome not running/accessible.
...
if ! curl -s --max-time 2 "$CDP_URL/json/version" > /dev/null 2>&1; then
    ...
    echo "   Run ./scripts/restart-chrome.sh first"
    exit 1
fi
